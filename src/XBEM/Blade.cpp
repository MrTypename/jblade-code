...
CVector *CBlade::m_pWakeNode;			//pointer to the VLM wake node array
CPanel  *CBlade::m_pWakePanel;			//pointer to the VLM Wake Panel array

void* CBlade::s_pMainFrame;		//pointer to the Frame window
void* CBlade::s_pMiarex;	//pointer to the Miarex Application window
void* CBlade::s_p3DPanelDlg;//pointer to the 3DPanel analysis dialog class
bool CBlade::s_bVLMSymetric;
...
void CBlade::ComputeVolumeInertia(CVector &CoG, double &CoGIxx, double &CoGIyy, double &CoGIzz, double &CoGIxz)
{
    //
...
void CBlade::ComputeBodyAxisInertia()
{
    //
...
void CBlade::CreateSurfaces(CVector const &T, double XTilt, double YTilt)
{
    //
    ...
    int j, nSurf;
    CVector PLA, PTA, PLB, PTB, T1, rot;
    ...
    for(j=0; j<=nSurf;j++)
    {
        if (j==0)               PitchAxis[j] = CVector(m_TPAxisX[j+1]-m_TPAxisX[j],m_TPAxisY[j+1]-m_TPAxisY[j],m_TPAxisZ[j+1]-m_TPAxisZ[j]);
        else if (j==nSurf)      PitchAxis[j] = CVector(m_TPAxisX[j]-m_TPAxisX[j-1],m_TPAxisY[j]-m_TPAxisY[j-1],m_TPAxisZ[j]-m_TPAxisZ[j-1]);
        else                    PitchAxis[j] = CVector(m_TPAxisX[j+1]-m_TPAxisX[j-1],m_TPAxisY[j+1]-m_TPAxisY[j-1],m_TPAxisZ[j+1]-m_TPAxisZ[j-1]);
    ...
    for (j=0; j<m_NPanel; j++)
    {
        if (fabs(m_TPos[j]-m_TPos[j+1]) > MinPanelSize)
        {
            ...
            PLA.x = m_TOffset[j]-m_TChord[j]*m_TFoilPAxisX[j];              PLB.x = m_TOffset[j+1]-m_TChord[j+1]*m_TFoilPAxisX[j+1];
            PLA.y = m_TPos[j];                                              PLB.y = m_TPos[j+1];
            PLA.z = -m_TChord[j]*m_TFoilPAxisZ[j];                          PLB.z = -m_TChord[j+1]*m_TFoilPAxisZ[j+1];
            PTA.x = PLA.x+m_TChord[j];                                      PTB.x = PLB.x+m_TChord[j+1];
            PTA.y = PLA.y;                                                  PTB.y = PLB.y;
            PTA.z = -m_TChord[j]*m_TFoilPAxisZ[j];                          PTB.z = -m_TChord[j+1]*m_TFoilPAxisZ[j+1];
            ...
            if (m_bIsOrtho) m_Surface[is].RotateOrtho();
            ...
            if(j>0)
            {
               T1 = CVector(m_TPAxisX[j]-m_TOffset[j], m_TPAxisY[j]-m_TPos[j],m_TPAxisZ[j]);
...
void CBlade::ComputeChords(int NStation)
{
    //
...
void CBlade::ComputeChords(int NStation, double *Chord, double *Offset, double *Twist)
{
    //
    // Calculates the chord lengths at each position of the NStation in LLT calculations
    //
...
bool CBlade::ExportAVLWing(QTextStream &out, int index, double x, double y, double z, double Thetax, double Thetay)
{
...
double CBlade::C4(double yob, double xRef)
{
...
double CBlade::Chord(double yob)
{
    double Chord = 0.0;
    double tau;
    double y;
...
double CBlade::Dihedral(double yob)
{
    int i;
    double y;
...
void CBlade::GetFoils(CFoil **pFoil0, CFoil **pFoil1, double y, double &t)
{
    int i;
    MainFrame *pMainFrame = (MainFrame*)s_pMainFrame;
...
double CBlade::Offset(double yob)
{
    double tau, y;
    double Offset = 0.0;
...
void CBlade::GetViewYZPos(double xrel, double y, double &yv, double &zv, int pos)
{
...
double CBlade::yrel(double SpanPos)
{
...
void CBlade::PanelTrefftz(double QInf, double Alpha, double *Mu, double *Sigma, int pos, CVector &Force, double &WingIDrag,
...
void CBlade::PanelSetBending(bool bThinSurface)
{
...
//			m_NYPanels[i] = qMin(100, m_NYPanels[i]);
        ...
        ComputeGeometry();
                ...
                if (ArchiveFormat>1011)
                {
                    for (i=0; i<=m_NPanel; i++)
                    {
                        ar >> f; m_TFoilPAxisX[i]=f;
                        ar >> f; m_TFoilPAxisZ[i]=f;
                    }
                }
                else
                {
...
void CBlade::ScaleSweep(double Sweep)
{
    //
...
void CBlade::PanelComputeOnBody(double QInf, double Alpha, double *Cp, double *Gamma, double &XCP, double &YCP,
...
void CBlade::PanelComputeViscous(double QInf, double Alpha, CWPolar *pWPolar, double &WingVDrag, bool bViscous, QString &OutString)
{
...
bool CBlade::IsWingPanel(int nPanel)
{
...
bool CBlade::IsWingNode(int nNode)
{
    for(int p=0; p<m_MatSize; p++)
    {
