...
//new code DM///
void CPolar::Serialize360(QDataStream &ar, bool bIsStoring, int ArchiveFormat)
{
   int i,n;
   float f;
   ...
   if (bIsStoring)
       ...
       WriteCString(ar, m_FoilName);
       WriteCString(ar, m_PlrName);
       WriteCString(ar,m_ParentPlrName);
       WriteCOLORREF(ar, m_Color);
       ...
       for (i=0;i<n;i++)
       {
           ar << (float)m_Alpha[i] << (float) m_Cl[i] << (float) m_Cd[i];
       }
       ...
       ReadCString(ar, m_FoilName);
       ReadCString(ar, m_PlrName);
       ReadCString(ar, m_ParentPlrName);
       ReadCOLORREF(ar, m_Color);
