...
//        {
//                QBEM *pBEM = (QBEM *) s_pBEM;
//                pBEM->m_ArcBall.GetMatrix();
//                pBEM->SetScale(m_rCltRect);
//                pBEM->Set3DScale();
//        }
//        ////////end new code DM/////////
...
//    const GLfloat light_ambient[4]  = {0.9,0.9,0.9, 0.8};
...
//    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    ...
    if (pBEM->m_pWing && pBEM->m_bResetglGeom) setSceneRadius(float(pBEM->m_pWing->m_TPos[pBEM->m_pWing->m_NPanel]));


    pBEM->GLDraw3D();
    pBEM->GLRenderView();
    if (pBEM->m_pctrlAxes->isChecked() && pBEM->m_pWing) GLDrawAxes(pBEM->m_pWing->m_PlanformSpan,pMainFrame->m_TextColor,1,2);
    ...
    if (pBEM->m_pctrlFoilNames->isChecked() && pBEM->m_pWing)
    {
        glColor3d(pMainFrame->m_TextColor.redF(),pMainFrame->m_TextColor.greenF(),pMainFrame->m_TextColor.blueF());
        for (int i=0;i<=pBEM->m_pWing->m_NPanel;i++)
            {
                QString strong;
                renderText(pBEM->m_pWing->m_PlanformSpan/10, pBEM->m_pWing->m_TPos[i], 0.0, pBEM->m_pWing->m_RFoil[i], pMainFrame->m_TextFont);
            }
    }

    if (pBEM->m_pctrlPositions->isChecked() && pBEM->m_pWing)
    {
        glColor3d(pMainFrame->m_TextColor.redF(),pMainFrame->m_TextColor.greenF(),pMainFrame->m_TextColor.blueF());
        ...
        {
            QString strong;
            renderText(-pBEM->m_pWing->m_PlanformSpan/10, pBEM->m_pWing->m_TPos[i], 0.0, QString(strong.number(pBEM->m_pWing->m_TPos[i],'f',3)+" m"), pMainFrame->m_TextFont);
        }
    }
...
void GLWidget::GLDrawAxes(double length, QColor AxisColor, int AxisStyle, int AxisWidth)
{
	...
	//XLabel
    renderText( l*1.1/6, 0.0, 0.0, "X", pMainFrame->m_TextFont);


	// Y axis____________
	glEnable (GL_LINE_STIPPLE);
	glBegin(GL_LINES);
        glVertex3d(0.0, -l/6, 0.0);
	...
	glDisable (GL_LINE_STIPPLE);
	//Y Label
    renderText( 0.0, l*1.01333, 0.0, "Y", pMainFrame->m_TextFont);
	...
	glDisable (GL_LINE_STIPPLE);
	//ZLabel
    renderText( 0.0, 0.0, l*1.1/6, "Z", pMainFrame->m_TextFont);
